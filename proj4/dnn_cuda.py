...
mylib = cdll.LoadLibrary('./lib_dnn_cuda.so')
...
class Conv2D(DnnNode):
    ...
    def run(self):
        ...
        if x_shape[0] % 16 != 0:
            x = np.vstack([x, np.zeros([16 - (x.shape[0] % 16), x.shape[1]])])
        if x_shape[1] % 16 != 0:
            x = np.hstack([x, np.zeros([x.shape[0], 16 - (x.shape[1] % 16)])])
        ...
        if w_shape[0] % 16 != 0:
            w = np.vstack([w, np.zeros([16 - (w.shape[0] % 16), w.shape[1]])])
        if w_shape[1] % 16 != 0:
