...
def postprocessing(predictions, input_image):
    ...
    for i in range(len(nms_predictions)):
        ...
        cv2.putText(input_image, best_class_name, (int(min(nms_predictions[i][0][0], nms_predictions[i][0][2]) - 1), int(min(nms_predictions[i][0][1], nms_predictions[i][0][3])) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

    return input_image

class YOLO2_TINY(object):

    def __init__(self, in_shape, onnx_path, debug):
        self.debug = debug
    ...
    def get_y2t_w(self):
        ...
        for i in range(0, 9):
            y2t_w[i] = {}
        y2t_w[0]["kernel"] = np.transpose(np.array(initializers["convolution_W"]).reshape(16, 3, 3, 3), perm)
        y2t_w[0]["biases"] = np.array(initializers["BatchNormalization_B"])
        y2t_w[0]["moving_mean"] = np.array(initializers["BatchNormalization_mean"])
        y2t_w[0]["moving_variance"] = np.array(initializers["BatchNormalization_variance"])
        y2t_w[0]["gamma"] = np.array(initializers["BatchNormalization_scale"])

        y2t_w[1]["kernel"] = np.transpose(np.array(initializers["convolution1_W"]).reshape(32, 16, 3, 3), perm)
        y2t_w[2]["kernel"] = np.transpose(np.array(initializers["convolution2_W"]).reshape(64, 32, 3, 3), perm)
        y2t_w[3]["kernel"] = np.transpose(np.array(initializers["convolution3_W"]).reshape(128, 64, 3, 3), perm)
        y2t_w[4]["kernel"] = np.transpose(np.array(initializers["convolution4_W"]).reshape(256, 128, 3, 3), perm)
        y2t_w[5]["kernel"] = np.transpose(np.array(initializers["convolution5_W"]).reshape(512, 256, 3, 3), perm)
        y2t_w[6]["kernel"] = np.transpose(np.array(initializers["convolution6_W"]).reshape(1024, 512, 3, 3), perm)
        y2t_w[7]["kernel"] = np.transpose(np.array(initializers["convolution7_W"]).reshape(1024, 1024, 3, 3), perm)

        for i in range(1, 8):
            y2t_w[i]["biases"] = np.array(initializers["BatchNormalization_B{}".format(i)])
            y2t_w[i]["moving_mean"] = np.array(initializers["BatchNormalization_mean{}".format(i)])
            y2t_w[i]["moving_variance"] = np.array(initializers["BatchNormalization_variance{}".format(i)])
            y2t_w[i]["gamma"] = np.array(initializers["BatchNormalization_scale{}".format(i)])

        y2t_w[8]["kernel"] = np.transpose(np.array(initializers["convolution8_W"]).reshape(125, 1024, 1, 1), perm)
    ...
    def build_graph(self, in_shape):
        ...
        bn0     =   self.g.create_batch_norm(conv0, y2t_w[0]["moving_mean"], y2t_w[0]["moving_variance"], y2t_w[0]["gamma"], 1e-5)
        ...
        bn3     =   self.g.create_batch_norm(conv3, y2t_w[1]["moving_mean"], y2t_w[1]["moving_variance"], y2t_w[1]["gamma"], 1e-5)
        ...
        bn6     =   self.g.create_batch_norm(conv6, y2t_w[2]["moving_mean"], y2t_w[2]["moving_variance"], y2t_w[2]["gamma"], 1e-5)
        ...
        bn9     =   self.g.create_batch_norm(conv9, y2t_w[3]["moving_mean"], y2t_w[3]["moving_variance"], y2t_w[3]["gamma"], 1e-5)
        ...
        bn12    =   self.g.create_batch_norm(conv12, y2t_w[4]["moving_mean"], y2t_w[4]["moving_variance"], y2t_w[4]["gamma"], 1e-5)
        ...
        bn15    =   self.g.create_batch_norm(conv15, y2t_w[5]["moving_mean"], y2t_w[5]["moving_variance"], y2t_w[5]["gamma"], 1e-5)
        ...
        bn18    =   self.g.create_batch_norm(conv18, y2t_w[6]["moving_mean"], y2t_w[6]["moving_variance"], y2t_w[6]["gamma"], 1e-5)
        ...
        bn20    =   self.g.create_batch_norm(conv20, y2t_w[7]["moving_mean"], y2t_w[7]["moving_variance"], y2t_w[7]["gamma"], 1e-5)
