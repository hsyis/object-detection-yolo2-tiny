...
#mylib = cdll.LoadLibrary('./lib_dnn_fp32.so')
mylib = CDLL(None)
...
class DnnInferenceEngine(object):
    def __init__(self, graph, precision):
        self.g = graph
        self.precision = np_dtype[precision]
        self.loadLibrary(self.precision);

    def loadLibrary(self, precision):
        soname = "lib_dnn.so"
        if precision == np_dtype["FP32"]:
            soname = "lib_dnn_fp32.so"
        elif precision == np_dtype["FP16"]:
            soname = "lib_dnn_fp16.so"
        elif precision == np_dtype["INT8"]:
            soname = "lib_dnn_int8.so"
        global mylib
        print("load library", soname)
        mylib = cdll.LoadLibrary("./" + soname)

    def run(self, tin):
        self.g.in_node.set_input(tin, self.precision)
        out = {}
