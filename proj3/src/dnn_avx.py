...
mylib = cdll.LoadLibrary('./lib_dnn_avx.so')
...
class Conv2D(DnnNode):
    ...
    def run(self):
        ...
        if x_shape[0] % 6 != 0:
            x = np.vstack([x, np.zeros([6 - (x.shape[0] % 6), x.shape[1]])])

        w = self.kernel.reshape([-1, self.n_ker])
        w_shape = w.shape
        if w.flags['F_CONTIGUOUS']:
            w = np.ascontiguousarray(w)
        if w_shape[1] % 16 != 0:
            w = np.hstack([w, np.zeros([w.shape[0], 16 - (w.shape[1] % 16)])])

        self.result = np.zeros((x.shape[0], w.shape[1]), dtype=np.float32)

        #self.result = x.dot(w)
        mylib.avx_mul_float(
            x.astype(np.float32).ctypes.data_as(POINTER(c_double)),
            w.astype(np.float32).ctypes.data_as(POINTER(c_double)),
            self.result.ctypes.data_as(POINTER(c_double)),
            x.shape[0],
            x.shape[1],
            w.shape[1])

        self.result = self.result[:x_shape[0], :w_shape[1]].reshape([self.n_in, self.h_out, self.w_out, self.n_ker])
